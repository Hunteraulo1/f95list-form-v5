...
>
  ...
  >
    ...
    >
      <div
        class="border-ext-border bg-ext-card sticky top-0 z-10 mx-0.5 rounded-b-xl border p-2 pb-8 text-center"
      >
        <p class="text-ext-secondary-foreground text-[0.65rem] leading-snug">
          Traduction détectée sur cette page
        </p>
        <div class="absolute top-8 right-0 flex w-full justify-center">
          <div
            class="border-ext-border bg-ext-card/40 text-ext-secondary-foreground hover:bg-ext-card/80 flex items-center justify-center rounded-full border-2 px-4 py-2 transition-opacity"
          >
      ...
      <div class="flex min-h-0 flex-1 flex-col gap-2 overflow-hidden">
        {#each displayGames as game (game.id)}
          <div
            class="bg-ext-card relative h-22 shrink-0 overflow-hidden rounded-md"
          >
            <img
              src={game.image}
              alt=""
            ...
            >
            <div
              class="text-ext-secondary-foreground relative flex h-full flex-col justify-end p-6 backdrop-brightness-90 transition hover:backdrop-brightness-100"
            >
              <p
            ...
            <div
              class="text-ext-secondary-foreground absolute top-1 right-1 rounded-full p-2 opacity-30"
            >
              <ClipboardCheck
                class="size-6"
                strokeWidth={2}
      ...
      <div class="absolute right-0 bottom-0 left-0 flex justify-center pb-4">
        <div
          class="border-ext-border bg-ext-card/80 text-ext-secondary-foreground hover:bg-ext-card bottom-2 z-10 mx-auto mt-auto w-fit rounded-md border-2 px-4 py-1.5 text-center text-xs font-medium shadow-sm"
        >
          Filtrer
        </div>
      </div>
    ...
    <div
      class="border-ext-border bg-ext-card flex h-14 w-full shrink-0 justify-around gap-2 border-t-4 p-1"
    >
      {#each navItems as item (item.icon)}
        <div
          class="text-ext-secondary-foreground relative flex flex-1 flex-col items-center justify-center rounded-md py-0.5 transition hover:opacity-100"
